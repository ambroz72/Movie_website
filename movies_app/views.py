...
def Signup(request):
    if request.method == 'POST':
        ...
        # If username is unique and passwords match, create the user
...
#                 fuser=User.objects.get(username=usernam) # for adding id in foriegn key column
#                 custm = Users(e_fname=Fname,e_lname=Lname,e_email=e_mail,e_user=fuser)
#                 custm.save()
...
#     efeed= Review.objects.all()
#     return render(request,'efeedback.html',{'usr':user,'efeeds':efeed})
...
#             addefeed.save()
#             atten.save()
#             sweetify.success(request,"User review has beign send")
